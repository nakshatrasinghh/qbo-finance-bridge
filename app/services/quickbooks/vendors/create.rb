  ...
  module Vendors
    class Create
      ...
      def call
        ...
        @post_attempted = true
        response = client.post("vendor", json: payload, params: { requestid: request_id })
        @quickbooks_entity_id = response.dig("Vendor", "Id").to_s
        if quickbooks_entity_id.blank?
          raise_unexpected!("QuickBooks did not return the created Vendor ID.")
        end
      ...
      def validate_input!
        unless display_name.length.between?(1, 100) && display_name.match?(NAME_PATTERN)
          raise_input!(
            "Vendor display name must be 1 to 100 characters without colon, tab, or newline."
          )
        end
      ...
      def validate_unique_name!
        duplicate =
          Query
            ...
            .call
            .any? { |vendor| vendor.display_name.casecmp?(display_name) }
        if duplicate
          raise_input!("An active QuickBooks Vendor with this display name already exists.")
        end
      end
      ...
      def readback(id)
        response = client.get("vendor/#{id}")
        raw = response["Vendor"]
        unless raw.is_a?(Hash)
          raise_unexpected!("QuickBooks returned invalid Vendor readback data.")
        end

        vendor = Details.from_payload(raw)
        valid =
          vendor.id == id && vendor.display_name == display_name &&
            (company_name.blank? || vendor.company_name == company_name) &&
            (email.blank? || vendor.email&.casecmp?(email)) &&
            (phone.blank? || vendor.phone == phone)
        unless valid
          raise_unexpected!("QuickBooks Vendor readback did not match the submitted record.")
        end
      ...
      def raise_input!(message)
        raise Error::Validation.new(
                message,
                code: "quickbooks_vendor_input_invalid",
      ...
      def raise_unexpected!(message)
        raise Error::UnexpectedResponse.new(
                message,
                code: "quickbooks_vendor_unexpected",
