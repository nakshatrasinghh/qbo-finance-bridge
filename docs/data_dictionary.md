...
Tokens are encrypted/decrypted by Active Record Encryption at the model boundary. The database receives ciphertext. Keys are not columns and are not stored in this table; they come from encrypted Rails credentials or the three `ACTIVE_RECORD_ENCRYPTION_*` environment variables. Losing those keys makes retained ciphertext unreadable. Rotating encryption keys requires a deliberate Rails encryption-key rotation plan and is outside the current phase.

## `quickbooks_sync_operations`

One row reserves one intended allowed create operation for one QuickBooks connection. Phases 13 and 14 expand the
fixed allowlist from Journal Entry to ten additional typed creates. This is a local
idempotency/audit record, not a native QuickBooks entity and not a second accounting ledger. QuickBooks remains
the source of truth.
...
### Phase 8 read-only API projection
...
`journal_entry_create`; Phase 13 does not expose request payloads or add a generic operation-history endpoint.
...
Successful results store only the normalized API projection after vendor readback. No Phase 13 row was created
during acceptance because no valid Phase 13 POST was sent.

Phase 17 created four succeeded rows: operation `25` Employee `400000001`, operation `26` TimeActivity
...
projection. No Phase 14 audit row was created during acceptance because no valid Phase 14 POST was sent.

Phase 16 created six succeeded rows using these fixed pairs: operation `4` Customer `58`, operation `5` Vendor
...
### Phase 9 browser-only CSV projections

Phase 9 adds no table or persisted field. The browser retains only the current bounded JSON responses and derives
two downloadable projections from the currently visible records:
...
### Phase 10 read parameters and pages

Phase 10 adds no table, persisted field, or migration. Both existing read APIs accept transient
`txn_date_from`, `txn_date_to`, `page`, and `per_page` values. Dates are exact ISO `YYYY-MM-DD`; page defaults to
1 and is capped at 10,000; page size defaults to 50 and is capped at 50. Responses echo normalized date filters
and return page metadata, but neither the request nor page state is stored.
...
### Phase 12 financial report projections

Phase 12 adds no table, persisted field, migration, cache, or audit row. Profit & Loss, Balance Sheet, and Cash
Flow are generated by QuickBooks on each GET and normalized transiently into report metadata, column definitions,
and flattened rows. Every row retains its QuickBooks kind/group/depth and ordered cells; realm-scoped IDs are
returned only when QuickBooks supplies them. Money cells are validated with `BigDecimal` and serialized as their
...
Phases 14 and 15 reuse this projection unchanged for General Ledger and Trial Balance. Trial Balance normalized
successfully. The corrected `reports/GeneralLedger` request also normalized successfully with 8 columns and 452
rows. Neither report payload is persisted.
...
| `last_verified_at` | timestamp | No | Time selected ID was last confirmed in a fresh active Account query | `2026-07-15T00:30:00Z` | Internal operational metadata | Local after QuickBooks read | Not null | Keep; later posting phase must define acceptable age/revalidation |
...
### Phase 3 reserved demo mapping

Phase 3 reuses the same schema rather than adding a demo-only table. One row has a fixed source identity so repeated actions update it instead of creating a second mapping:
...
The unique source-identity index makes this mapping an updateable reservation locally. It is not a full remote idempotency ledger: QuickBooks name uniqueness plus query-before-create provide the bounded Phase 3 protection. Future arbitrary financial writes require explicit idempotency/audit records rather than copying this name-based approach.

### Framework-managed schemas

Generated production configurations for Solid Cache, Solid Queue, and Solid Cable remain present. The current application does not prepare or use them for QuickBooks behavior, and their framework tables are not application-domain records.
