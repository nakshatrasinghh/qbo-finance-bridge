...
- Status: Accepted for sandbox phases
- Date: 2026-07-14

## Context

The application must make QuickBooks behavior visible to a developer learning the integration: host, method, realm-scoped path, minor version, bearer header, JSON boundary, OAuth refresh, bounded retry, timeout, Intuit fault status, and correlation metadata. Phase 1 needs one CompanyInfo GET and later phases will add entity-specific reads/writes.
...
- GET now and POST when an approved write phase requires it;
...
- **Official/third-party QuickBooks Ruby SDK:** rejected for early phases because it obscures the direct REST learning objective, adds a larger compatibility/maintenance surface, and is unnecessary for one read.
...
- Production URLs are known constants but production selection hard-fails until a dedicated readiness phase.
...
- Mature implementation review: Mastodon `app/lib/request.rb` at commit `d70f1f983c945b3aa4c2089e540c67d706d762d9`; Discourse OAuth files listed in `docs/reference_review.md` at commit `939248f3690e3557207a3e4cd90bb7760201d4b0`.
